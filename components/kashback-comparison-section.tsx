...
const KASHBACK_LOGO_SRC = encodeURI(
  "/LOGO OFICIAL/SVG_VERSÕES LOGO HORIZONTAL/ALTERNATIVO KMILLION_LARANJA.svg"
);
...
const comparisonRows = [
  {
    ...
    label: "Genérico",
    market: "Genérico",
    kmillion: "Segmentado por perfil",
  },
  {
    icon: Database,
    label: "Sem dados",
    market: "Sem dados",
    kmillion: "Dados em tempo real",
  },
  {
    icon: ShoppingBag,
    label: "Compra única",
    market: "Compra única",
    kmillion: "Recompra planejada",
  },
  {
    icon: Plug,
    label: "Apenas plugado",
    market: "Apenas plugado",
    kmillion: "Pilotado com inteligência promocional",
  },
  {
    icon: BarChart3,
    label: "Sem mensuração",
    market: "Sem mensuração",
    kmillion: "ROI visível por ação",
  },
] as const;

export function KashbackComparisonSection(): ReactNode {
  return (
    <section
      id="comparacao"
    ...
    >
      ...
      >
        <svg
          className="absolute top-[16%] left-[-8%] h-[42rem] w-[42rem] opacity-60"
          viewBox="0 0 800 800"
          fill="none"
        >
          <path
        ...
        <svg
          className="absolute right-[-6%] bottom-[-10%] h-[46rem] w-[46rem] opacity-55"
          viewBox="0 0 800 800"
          fill="none"
        >
          <path
      ...
      <div className="relative mx-auto max-w-[1500px]">
        ...
        >
          <h2 className="text-[clamp(2.95rem,6vw,6rem)] leading-[0.94] font-semibold tracking-[-0.075em] text-white">
            <span className="block">Cashback tradicional</span>
          ...
          <div className="relative hidden md:block">
            ...
            {comparisonRows.map((row, index) => {
              ...
              return (
                <div
                  key={row.label}
                  className={`grid grid-cols-[1.15fr_0.82fr_1fr] ${index === comparisonRows.length - 1 ? "" : "border-b border-white/10"}`}
                >
                  <div className="flex items-center gap-4 px-8 py-8">
                    <Icon
                      ...
                      strokeWidth={1.85}
                    />
                    <div className="text-[1.02rem] leading-snug font-semibold text-white sm:text-[1.15rem]">
                      {row.label}
                    </div>
                  </div>
                  ...
                  <div className="border-l border-[#FF7A1A]/28 px-8 py-8">
                    <div className="flex h-full items-center gap-4 rounded-[1.45rem] border border-[#FF7A1A]/24 bg-[linear-gradient(180deg,rgba(255,122,26,0.18),rgba(0,0,0,0.2))] px-5 py-5 shadow-[0_0_0_1px_rgba(255,122,26,0.05),0_0_42px_rgba(255,122,26,0.1)]">
                      ...
                      <div className="max-w-[20ch] text-[1rem] leading-snug font-medium text-white sm:text-[1.1rem]">
                        {row.kmillion}
                      </div>
                    </div>
                  </div>
